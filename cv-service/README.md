...
## Real test run (input footage: campus gate camera, 2560×1440 @ 20fps)

Run against a 239-frame/~12s clip with one car (plate `AAL 988`) and one motorcycle (plate `BAG 9976`) passing an underground gate entrance. After both fixes above, the corrected run produces exactly **3 detection events** (down from 6 pre-fix):
...
- **One spurious low-confidence read ("1") — found and fixed**: came from a 20×12px plate-detector false positive on a distant/small vehicle, too small to physically contain a legible plate. Fixed with a minimum-box-size filter in `YoloPlateDetector` (`min_plate_width_px`/`min_plate_height_px`, defaults 24×10px — below the smallest genuine plate box seen in this footage, ~51×33px). Not yet re-tested end-to-end on this clip.
- **Performance (CPU-only, no GPU):** ~1.3–1.4s/processed-frame at 2560×1440 including PaddleOCR calls (measured from timestamped logs: 200 frames in ~285s once models were warm), ~5.5-6 minutes wall-clock for this 12s/239-frame clip. PaddleOCR also downloads ~140MB of its own OCR models on first use, separate from `requirements.txt` — a one-time cost, not counted above. Even at this rate, one frame alone takes roughly half of NFR-1's entire 2-3s detection-to-alert budget — nowhere close once you add the backend's own consume/match/write/push latency on top. Phase 2 will need a GPU, `--frame-skip`, and/or a lower processing resolution to have any real chance of meeting NFR-1; worth load-testing early rather than assuming CPU is viable at this resolution.
